...
| P7 | 策略步态质量 | MuJoCo 1.0 m/s rollout（Phase 5.5 自动）经 check_policy_gait.py：臂反相 corr ≤ −0.50；肘 p95 ∈ [8°,70°]；腰 yaw 摆 ≤ 40°；臂-对侧腿耦合 ≥ +0.20；髋摆幅比 ∈ [0.7,1.4]；rollout 存活 ≥8s 不摔 | 动作像不像人（P1–P6 只测过程健康；v29 13/13 PASS 仍同相摆臂+肘折叠，实测 anti +0.91/肘 89.9°/耦合 −0.68）|
...
## 1.5 P7 阈值校准记录（2026-09-10，历史策略实测）

| 策略 | anti | 肘 p95 | 耦合 | 判定 |
|---|---|---|---|---|
| v27 | −0.94 | 56.6° | −0.12 | FAIL(P7d) |
| v28 model_3999 | +0.96 | 47.0° | −0.14 | FAIL(P7a,P7d) |
| v29e4b | +0.91 | 89.9° | −0.68 | FAIL(P7a,P7b,P7d) |
| 参考数据 | −0.92~−0.99 | ~20° | +0.8~+0.99 | — |

同相摆臂自 v28（v28b 臂部奖励改动后）出现、v29 加剧并叠加肘折叠。
P7 FAIL 线留了策略侧余量（参考值更严）；TARGET 线 anti ≤ −0.70 /
耦合 ≥ +0.45。v31 首测后按实测收紧。
