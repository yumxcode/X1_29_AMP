...
# 节奏弧终章（v63-v67h，2026-09-23，节奏门=本 AMP 栈结构极限）

> 目标（contract rev1-4）：100Hz 控制下步幅/步频满足人类行走节奏（P8 门），sim2sim
> 验证，长周期观测 timer park。详录 V64/V66/V67_REPORT + GOAL_RHYTHM.md + RHYTHM_GATES.md。

## 判定（诚实终态）：节奏门未达成——十五轮全通道穷尽

| 通道 | 轮次 | 终态 |
|---|---|---|
| 奖励工程 | v64（事件核 0.6）/v64b（空中时长核 0.3）/v64c（运动学正弦参考 0.4） | 收入全部冻结于既有微步部分分；sim2sim 拖步化 |
| 数据门控（entrenched） | v65/v65b（VMATCH 时间+motion 双门控） | cadence 1.85→1.89Hz 噪声级 |
| 从头训（旧库） | v66/v66b（+节奏三件套，补满 4000） | 3.83Hz 快踏 |
| **demo 整库替换** | **v67d/e（+三件套）→ v67f/h（摘除，收敛 986）** | **3.83-3.92Hz 快踏+全拖步（髋摆 27-44°，脚不离地）** |
| mjlab 域内 | 探针 v3/v4/v5 | infra 阻塞（镜像预算） |
| 物理可行性 | 本地逆动力学 | 人类节奏力矩低 10×——**非硬件限制** |

**根因（最终）**：快微步是平衡学习的容易吸引子（不承诺单支撑）；接触时序模仿信号在
本 AMP 架构不可有效传导（与 H1 甩击、低速拖步族同族的第三次跨域接触时序取证）。
v67f 隔离轮证明节奏三件套是收敛干扰项（同期 ep_len 703-850 vs 599）且不改变终态。

## 关键资产（全部入库）

- P8 节奏门（rhythm_gates.py，人类参考实测校准）+ gait_metrics R 快读
- VMATCH 全链（sample_motions/times_speed_gated）+ x1_lab_v67 overground 库（gate PASS）
- 节奏三奖励（默认 0）+ 干跑 29/29 + physics_rhythm_feasibility.py
- on-pod harness final_checkpoint 修复（数字优先）
- 账号池消耗记录：kobay82044(不足)→kagogap318(6 轮耗尽)→yihovow790(5 轮耗尽)→rahobaf479

## 发布矩阵不变

**m10500@100Hz（v63 弧）= 最优部署点（13/14 门；P8 节奏为其已记录缺口）**。
解锁方向：mjlab 域内（jax+mjlab 预装镜像）、相位时钟参考跟踪（footfall 先验）、
外部 overground 源扩展。
