...
def main():
    ...
    # v28: *_mirror.pkl are FK-verified L/R-mirrored twins (generated by
    # roboparty_train/mirror_lab_motions.py) — they duplicate the source
    # motion reflected, so checking them against per-file structural rules
    # designed for the 14 originals would mis-fire. Skip; symmetry of the
    # mirrored set is guaranteed by construction (FK mirror err < 1 mm).
    lab_files = sorted(p for p in lab_dir.glob("*.pkl")
                       if not p.stem.endswith("_mirror")) if lab_dir.exists() else []
