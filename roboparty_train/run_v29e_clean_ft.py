...
Resumes from the v29e4b clean-phase checkpoint (model_10496,
TASK_20260910_037) for +1000 iterations with the v29 randomization DISABLED
...
Checkpoint sourcing (in priority order):
  1. repo-embedded roboparty_train/checkpoints/model_10496.pt (16.9MB)
  2. platform resume mount (repo root model_10496*.pt — same-account OSS)
...
_CKPT = _HERE / "checkpoints" / "model_10496.pt"
_OSS_URL = "https://limx-gradmotion.oss-cn-beijing.aliyuncs.com/upload%2F2026%2F9%2F10%2Fmodel_10496_20260910074416A752.pt?OSSAccessKeyId=LTAI5tMec8RQN1nZuRkVMgxz&Expires=1789603124&Signature=%2FbNXdufmEMfywOsD5tVyx0%2BmPic%3D"

if _CKPT.is_file():
    os.environ["X1_RESUME_CKPT"] = str(_CKPT)
else:
    # fall back to the platform mount if present, else pull from OSS
    mounted = sorted((p for p in _HERE.parent.glob("model_10496*.pt")))
    if mounted:
